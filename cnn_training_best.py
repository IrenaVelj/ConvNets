...
#from keras.layers.advanced_activations import Leakyelu
...
def my_model(Inputs, nclasses, nregressions, otheroption, dropoutRate=0.01, momentum=0.8):
...
#Danse Layers: 4
    ...
    print(type(Inputs[0]))
    x = Inputs[0]
    x = BatchNormalization(mode=0, axis=4, momentum=0.9)(x)
    print(x)
    x = Convolution3D(32, kernel_size=(1,1,2), strides=(1,1,1), activation='elu', kernel_initializer='lecun_uniform', kernel_regularizer=l2(0.0001))(x)
    x = Dropout(dropoutRate)(x)
    x = Convolution3D(64, kernel_size=(1,1,5), strides=(1,1,1), activation='elu', kernel_initializer='lecun_uniform', kernel_regularizer=l2(0.0001))(x)
    ...
    for i in range(1):
        x = Convolution3D(32, kernel_size=(1, 1, 2), strides=(1, 1, 1), activation='elu',
                      kernel_initializer='lecun_uniform',kernel_regularizer=l2(0.0001))(x)
        x = BatchNormalization(momentum=0.9)(x)
        x = Dropout(dropoutRate)(x)

    # layer, eta, phi : 38 x 5 x 5
    x = Convolution3D(32, kernel_size=(2, 2, 4), strides=(1, 1, 4), activation='elu',padding='valid',
                      kernel_initializer='lecun_uniform', kernel_regularizer=l2(0.0001))(x)
    x = BatchNormalization(momentum=0.9)(x)
    x = Dropout(dropoutRate)(x)
    ...
    x = Flatten()(x)
    x = Dense(64, activation='elu', kernel_regularizer=l2(0.0001))(x)
    x = BatchNormalization(momentum=0.9)(x)
    x = Dropout(0.1)(x)
